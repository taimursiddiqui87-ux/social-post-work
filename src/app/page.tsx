...
interface DraftRow {
  ...
  created_at: string;
  item_id: string;
  items: { title: string; url: string; relevance_score: number | null; published_at: string | null } | null;
}
...
export default async function QueuePage() {
  ...
  const [draftsRes, postedCountRes] = await Promise.all([
    sb.from("drafts")
      .select("id,platform,body,hashtags,hook,status,created_at,item_id,items(title,url,relevance_score,published_at)")
      .eq("status", "pending")
  ...
  return (
    <div className="space-y-8">
      <header className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-[28px] font-semibold tracking-tight">Queue</h1>
          <p className="mt-1 text-sm text-neutral-500">Review AI-generated drafts. Copy and post when ready.</p>
        </div>
        <Toolbar />
      </header>
      ...
      {groups.size === 0 ? (
        <div className="rounded-2xl border border-dashed border-neutral-800/80 p-16 text-center">
          ...
          <p className="mt-1 text-sm text-neutral-500">Click <strong className="text-neutral-300">Fetch news</strong>, then <strong className="text-neutral-300">Generate drafts</strong>.</p>
        </div>
      ) : (
